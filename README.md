...
| `npm run build && npm run local:start` | Compiled `dist/` | ✓ auto-bypass | E2E tests against the compiled artifact |
| `npm run build && npm run prod:start` | Compiled `dist/` | ✗ | Production / prod simulation |
...
**`local:start`** serves the pre-built static files and also keeps test routes enabled — which is what Cypress relies on for session setup. Use this to run E2E tests against the real compiled artifact (`npm run cy:run:user` / `npm run cy:run:guest`).

**`prod:start`** runs the compiled bundle with test routes stripped. The Log In button triggers real Replit Auth (requires `REPL_ID` in env). Use this to simulate or run a production deployment.
