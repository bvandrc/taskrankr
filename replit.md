...
- **Vite duplicate-React / Invalid hook call**: `vite.config.ts` sets `resolve.dedupe: ['react', 'react-dom']` and `optimizeDeps.include: ['workbox-window', 'es-toolkit/compat']`. Root cause: these packages are not discovered during Vite's initial module-graph crawl, so they trigger a mid-session full-page reload + re-bundle the first time they're needed. That reload races with module loading and can land two React instances in the graph simultaneously (causing "Invalid hook call" errors and aborting in-flight Cypress requests). Pre-including them prevents the mid-session reload entirely. Do not remove any of these options. If a new package causes the same symptom, add it to `optimizeDeps.include`.
